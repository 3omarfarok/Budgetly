...
export default function InventoryTable({
  ...
  return (
    <div className="overflow-x-auto rounded-[28px] border border-ios-border bg-ios-surface shadow-sm dark:border-white/8">
      <table
        className="min-w-[980px] w-full border-collapse text-right text-ios-dark dark:text-white"
        dir="rtl"
      >
        ...
        <tbody className="divide-y divide-ios-border dark:divide-white/8">
          {items.map((item) => {
            ...
            return (
              <tr
                key={item._id}
                className={`${rowClassName} transition-colors hover:bg-ios-light/70 dark:hover:bg-white/4`}
              >
                <td className="px-4 py-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${statusMeta.surfaceClassName}`}
                    >
                      {isUrgent ? (
                        <TriangleAlert
                          className={`h-4 w-4 ${statusMeta.accentClassName}`}
                        />
                      ) : (
                        <Archive
                          className={`h-4 w-4 ${statusMeta.accentClassName}`}
                    ...
                    <div className="min-w-0">
                      <p
                        className="truncate text-sm font-bold text-ios-dark dark:text-white"
                        title={item.name}
                      >
                        {item.name}
                      </p>
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex cursor-pointer items-center gap-1 rounded-md border border-ios-border bg-ios-light/70 px-3 py-2 text-xs font-bold text-ios-dark transition-colors hover:bg-ios-light disabled:cursor-not-allowed disabled:opacity-50 dark:border-white/10 dark:bg-white/6 dark:text-white dark:hover:bg-white/10"
